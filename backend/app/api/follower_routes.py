...
def follows():
    if request.method == "POST":
        ...
        follow = Follower(follower_by_id=current_user.id, creator_id=follow_id)
...
@follow_routes.route("/follow/view")
def view_follows():
    follows_id = request.json.get("follow")
    follows = Follower.query.filter(
        Follower.follower_by_id == follows_id, Follower.verified == True
    ).all()
    followed_by = Follower.query.filter(
        Follower.following_id == follows_id, Follower.verified == True
    ).all()
    followed_by_requests = Follower.query.filter(
        Follower.following_id == follows_id, Follower.verified == False
    ).all()
    ...
    return {
        ...
        "followed_by_request": followed_by_requests,
    }, 200
