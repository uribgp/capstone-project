    ...
    return (
        <div>
            ...
            {profile.user.payment_methods ?
                <div>
                    {profile.user.payment_methods.map((payment) => <PaymentMethod title={payment.title} cost={payment.cost} 
                    description={payment.description} key={payment.id} id={payment.id} userId={payment.user_id} coachId={payment.user_id}/>)}
                </div>
             : null}
